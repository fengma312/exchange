...
public class MatchCore
{
    ...
    public MatchCore(ServiceList service_list, MatchModel model)
    {
    ...
    public (List<OrderBook> bid, List<OrderBook> ask) GetOrderBook()
    {
        var bids = from b in fixed_bid
                   group b by new { b.market_id, b.symbol, b.price } into g
                   select new OrderBook { market = g.Key.market_id, symbol = g.Key.symbol, price = g.Key.price ?? 0, amount = g.Sum(p => p.unsold), count = g.Count(), direction = E_OrderSide.buy, last_time = g.Max(p => p.deal_last_time ?? DateTimeOffset.UtcNow) };
        var asks = from a in fixed_ask
                   group a by new { a.market_id, a.symbol, a.price } into g
                   select new OrderBook { market = g.Key.market_id, symbol = g.Key.symbol, price = g.Key.price ?? 0, amount = g.Sum(p => p.unsold), count = g.Count(), direction = E_OrderSide.sell, last_time = g.Max(p => p.deal_last_time ?? DateTimeOffset.UtcNow) };
    ...
    public (List<Orders> orders, List<Deal> deals, List<Orders> cancels) Match(Orders order)
    {
        ...
        decimal? price = null;
        if (order.market_id != this.model.info.market_id || order.unsold <= 0 || order.state == E_OrderState.completed || order.state == E_OrderState.cancel)
        {
    ...
    public (List<Orders> orders, List<Deal> deals, List<Orders> cancels) MatchByArchives(Orders order, int archives)
    {
        ...
        List<Orders> cancels = new List<Orders>();
        decimal? price = null;
        if (order.market_id != this.model.info.market_id || order.unsold <= 0 || order.state == E_OrderState.completed || order.state == E_OrderState.cancel || archives <= 0)
        {
            return (orders, deals, cancels);
        }
        if (order.state == E_OrderState.not_match && order.trigger_hanging_price > 0)
    ...
    public decimal? CreateDeal(Orders bid, Orders ask, decimal price, E_OrderSide trigger_side, List<Orders> orders, List<Deal> deals, List<Orders> cancels)
    {
        ...
        Deal deal = new Deal()
        {
            trade_id = ServiceFactory.instance.worker.NextId(),
            trade_model = bid.trade_model,
            market = this.model.info.market_id,
        ...
        if (trigger_order.Count > 0)
        {
            ...
            // this.model.mq_helper.MqSend(this.service_key.GetMqOrderPlace(this.model.info.market), Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(call_req)));
        }
        return price;
    }
