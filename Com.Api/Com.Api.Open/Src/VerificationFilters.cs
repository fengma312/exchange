...
public class VerificationFilters : Attribute, IAuthorizationFilter
{
    ...
    public void OnAuthorization(AuthorizationFilterContext context)
    {
        ...
        RedisValue rv = ServiceFactory.instance.redis.HashGet(ServiceFactory.instance.service_key.GetRedisApiKey(), key);
        if (rv.IsNullOrEmpty)
        {
            using (var scope = ServiceFactory.instance.service_base.provider.CreateScope())
            {
                using (DbContextEF db = scope.ServiceProvider.GetService<DbContextEF>()!)
                {
                    ...
                    if (userapi != null)
                    {
                        ServiceFactory.instance.redis.HashSet(ServiceFactory.instance.service_key.GetRedisApiKey(), key, JsonConvert.SerializeObject(userapi));
                    }
                }
            }
        }
