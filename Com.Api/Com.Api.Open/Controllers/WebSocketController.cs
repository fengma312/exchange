...
public class WebSocketController : ControllerBase
{
    ...
    public WebSocketController(IServiceProvider provider, IDbContextFactory<DbContextEF> db_factory, IConfiguration configuration, IHostEnvironment environment, ILogger<MainService> logger)
    {
        ...
        {
            this.mq_helper = new HelperMq(this.service_base, ServiceFactory.instance.connection_factory);
            this.service_base.logger.LogInformation(ex, $"mq创建通道报错,使用mq连接重新创建通道");
        }
    }
    ...
    public async Task<IActionResult> Push()
    {
        if (HttpContext.WebSockets.IsWebSocketRequest)
        {
            ...
            {
                this.service_base.logger.LogInformation(ex, $"websocket报错");
            }
        }
    ...
    private void Subscribe(WebSocket webSocket, string str, Dictionary<string, (string queueName, string consume_tag)> channel, ref long uid)
    {
        ...
        if (req.op == E_WebsockerOp.login)
        {
            ...
            if (req.args != null && req.args.Count > 0 && !string.IsNullOrWhiteSpace(req.args[0].data))
            {
                try
                {
                    ...
                    if (req_login != null)
                    {
                        UsersApi? users_api = this.service_list.service_user.GetApi(req_login.api_key);
                        if (users_api != null)
                        {
        ...
        else if (req.op == E_WebsockerOp.subscribe)
        {
            foreach (ReqChannel item in req.args)
            {
                ...
                {
                    Market? market = this.service_list.service_market.GetMarketBySymbol(item.data);
                    if (market == null)
                    {
                        continue;
                    }
                    ...
                    {
                        string key = this.service_list.service_key.GetMqSubscribe(item.channel, market.market_id);
                        if (login_channel.Contains(item.channel))
                        {
                            key = this.service_list.service_key.GetMqSubscribe(item.channel, uid);
                        }
