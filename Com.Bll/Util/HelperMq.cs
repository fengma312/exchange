...
public class HelperMq
{
    ...
    public HelperMq(ServiceBase service_base, ConnectionFactory connectionFactory)
    {
    ...
    public bool MqSend(string queue_name, byte[] body)
    {
        ...
        {
            this.service_base.logger.LogError(ex, "MQ 简单的队列 发送消息");
            return false;
        }
        return true;
    }
    ...
    /// <returns></returns>
    public async Task<(string queue_name, string consume_tag)> Simple(string queue_name, Channel<byte[]> channel)
    {
        IBasicProperties props = i_model.CreateBasicProperties();
        props.DeliveryMode = 2;
        i_model.QueueDeclare(queue: queue_name, durable: true, exclusive: false, autoDelete: false, arguments: null);
        ...
        consumer.Received += (model, ea) =>
        {
            if (channel.Writer.TryWrite(ea.Body.ToArray()))
            {
                i_model.BasicAck(deliveryTag: ea.DeliveryTag, multiple: true);
            }
            else
    ...
    public bool MqTask(string queue_name, byte[] body)
    {
        ...
        {
            this.service_base.logger.LogError(ex, "MQ 发布工作任务");
            return false;
        }
        return true;
    }
    ...
    public bool MqPublish(string exchange, string message)
    {
        ...
        {
            this.service_base.logger.LogError(ex, "MQ发布消息错误");
            return false;
        }
        return true;
    }
    ...
    public void MqDeletePurge()
    {
        ...
        {
            this.service_base.logger.LogError(ex, "清除mq队列失败");
        }
    }
    ...
    public void MqDelete(string queueName)
    {
        ...
        {
            this.service_base.logger.LogError(ex, "删除mq队列失败");
        }
    }
    ...
    public void MqDeleteConsumer()
    {
        ...
        {
            this.service_base.logger.LogError(ex, "删除mq消费者失败");
        }
    }
