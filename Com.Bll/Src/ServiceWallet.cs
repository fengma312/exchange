...
public class ServiceWallet
{
    ...
    public ServiceWallet(ServiceBase service_base)
    {
            ...
            using (DbContextEF db = this.service_base.db_factory.CreateDbContext())
            {
                using (var transaction = db.Database.BeginTransaction(isolationLevel))
                {
                    ...
                    catch (DbUpdateConcurrencyException ex)
                    {
                        ...
                        this.service_base.logger.LogError(ex, "Transfer(并发):" + ex.Message);
                    ...
                    {
                        transaction.Rollback();
                        this.service_base.logger.LogError(ex, "FreezeChange:" + ex.Message);
                        res.code = E_Res_Code.db_error;
                        res.msg = "钱包类型之间划转出错";
                        return res;
                    }
        ...
        //using (var scope = ServiceFactory.instance.constant.provider.CreateScope())
        {
            using (DbContextEF db = this.service_base.db_factory.CreateDbContext())
            {
                using (var transaction = db.Database.BeginTransaction(isolationLevel))
                {
                    ...
                    {
                        transaction.Rollback();
                        this.service_base.logger.LogError(ex, "FreezeChange(并发):" + ex.Message);
                        return false;
                    }
                    catch (Exception ex)
                    {
            ...
            using (DbContextEF db = this.service_base.db_factory.CreateDbContext())
            {
                using (var transaction = db.Database.BeginTransaction(isolationLevel))
                {
                    try
                    {
                        ...
                        {
                            this.service_base.logger.LogError($"{market.symbol}:交易对没有找到结算账户");
                        ...
                        foreach (Deal item in deals)
                        {
                            ...
                            {
                                this.service_base.logger.LogError($"{market.symbol}:用户:{item.bid_uid}/{item.ask_uid},未找到交易账户钱包");
                                return (false, runnings_fee, runnings_trade);
                            }
                            temp_base = 0;
                            temp_quote = 0;
                    ...
                    {
                        transaction.Rollback();
                        this.service_base.logger.LogError(ex, "Transaction(并发):" + ex.Message);
                        runnings_fee.Clear();
                        return (false, runnings_fee, runnings_trade);
                    }
                    catch (Exception ex)
                    {
                        transaction.Rollback();
                        this.service_base.logger.LogError(ex, market.symbol + ":Transaction," + ex.Message);
                        runnings_fee.Clear();
                        return (false, runnings_fee, runnings_trade);
                    }
                }
    ...
    //                 {
    //                     transaction.Rollback();
    //                     this.service_base.logger.LogError(ex, "TansferAvailable(并发):" + ex.Message);
    //                     return false;
    //                 }
    //                 catch (Exception ex)
    //                 {
    //                     transaction.Rollback();
    //                     this.service_base.logger.LogError(ex, "TansferAvailable" + ex.Message);
    //                     return false;
    //                 }
    //             }
    //         }
    ...
    public RunningFee AddRunningFee(long relation_id, E_RunningType type, E_WalletType wallet_type_from, E_WalletType wallet_type_to, decimal amount, Wallet wallet_from, Wallet wallet_to, string remarks)
    {
        ...
        {
            running_id = ServiceFactory.instance.worker.NextId(),
    ...
    private void PushWallet(List<Wallet> wallets, string remark)
    {
        ...
        try
        {
            ...
            foreach (var item in wallet_uid)
            {
                ...
                ServiceFactory.instance.mq_helper.MqPublish(this.service_key.GetMqSubscribe(E_WebsockerChannel.assets, item.user_id), JsonConvert.SerializeObject(res, new JsonConverterDecimal()));
            }
        }
        catch (System.Exception ex)
        {
            this.service_base.logger.LogError(ex, "PushWallet" + ex.Message);
        }
        stopwatch.Stop();
        this.service_base.logger.LogTrace($"计算耗时:{stopwatch.Elapsed.ToString()};:Mq=>资金变更推送,{remark}");
    }
