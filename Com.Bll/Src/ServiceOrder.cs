...
public class ServiceOrder
{
    ...
    /// </summary>
    /// <returns></returns>
    private readonly ServiceUser service_user;

    /// <summary>
    /// 初始化
    /// </summary>
    /// <param name="service_base">基础服务</param>
    public ServiceOrder(ServiceBase service_base)
    {
    ...
    /// <returns></returns>
    public Res<List<BaseOrdered>> PlaceOrder(string symbol, long uid, string user_name, string ip, List<BaseOrder> orders)
    {
        ...
        foreach (var item in orders)
        {
            ...
            order.order_id = ServiceFactory.instance.worker.NextId();
        ...
        this.service_base.logger.LogTrace($"计算耗时:{ServiceFactory.instance.stopwatch.Elapsed.ToString()};{info.symbol}:挂单=>校验/冻结资金{temp_order.Count}条挂单记录");
        ServiceFactory.instance.stopwatch.Restart();
        int dbcount = 0;
        ...
        ServiceFactory.instance.stopwatch.Stop();
        this.service_base.logger.LogTrace($"计算耗时:{ServiceFactory.instance.stopwatch.Elapsed.ToString()};{info.symbol}:挂单({ip})=>插入{dbcount}条订单到DB");
        ServiceFactory.instance.stopwatch.Restart();
        ReqCall<List<Orders>> call_req = new ReqCall<List<Orders>>();
        call_req.op = E_Op.place;
        call_req.market = info.market_id;
        call_req.data = temp_order;
        ServiceFactory.instance.mq_helper.MqSend(this.service_key.GetMqOrderPlace(info.market_id), Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(call_req)));
        ServiceFactory.instance.stopwatch.Stop();
        this.service_base.logger.LogTrace($"计算耗时:{ServiceFactory.instance.stopwatch.Elapsed.ToString()};{info.symbol}:挂单=>插入{call_req.data.Count}条订单到Mq");

        res.code = E_Res_Code.ok;
        res.msg = "挂单成功";
        res.data.AddRange(temp_order);
        this.stopwatch.Stop();
        this.service_base.logger.LogTrace($"计算耗时:{this.stopwatch.Elapsed.ToString()};{info.symbol}:挂单=>总耗时.{call_req.data.Count}条订单");
        return res;
    }
    ...
    public Res<bool> CancelOrder(string symbol, long uid, int type, List<long> order)
    {
        ...
        req.op = op;
        req.market = info.market_id;
        req.data = (uid, order);
        res.data = ServiceFactory.instance.mq_helper.MqSend(this.service_key.GetMqOrderPlace(info.market_id), Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(req)));
        if (res.data)
        {
            res.code = E_Res_Code.ok;
        }
    ...
    public bool UpdateOrder(List<Orders> data)
    {
        //using (var scope = ServiceFactory.instance.constant.provider.CreateScope())
        {
            using (DbContextEF db = this.service_base.db_factory.CreateDbContext())
            {
                ...
                {
                    this.service_base.logger.LogError($"更新订单失败:{ex.Message}");
                    return false;
                }
                return true;
            }
    ...
    public bool PushOrderToMqRedis(long market)
    {
        ...
        if (orders.Count > 0)
        {
            ...
            ServiceFactory.instance.mq_helper.MqSend(this.service_key.GetMqOrderPlace(market), Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(call_req)));
        }
        return true;
    }
    ...
    private List<Orders> GetNoCompletedOrders(long market)
    {
        //using (var scope = ServiceFactory.instance.constant.provider.CreateScope())
        {
            using (DbContextEF db = this.service_base.db_factory.CreateDbContext())
            {
                ...
                orders.AddRange(db.OrderBuy.Where(P => P.side == E_OrderSide.buy && P.market_id == market && (P.state == E_OrderState.unsold || P.state == E_OrderState.partial)).AsNoTracking().ToList());
                orders.AddRange(db.OrderSell.Where(P => P.side == E_OrderSide.sell && P.market_id == market && (P.state == E_OrderState.unsold || P.state == E_OrderState.partial)).AsNoTracking().ToList());
    ...
    /// <returns></returns>
    public Res<List<BaseOrdered>> GetOrder(long uid, string? symbol = null, List<long>? order_id = null, List<E_OrderState>? state = null, DateTimeOffset? start = null, DateTimeOffset? end = null, int skip = 0, int take = 50)
    {
        Res<List<BaseOrdered>> res = new Res<List<BaseOrdered>>();
